...
mpl.rcParams['axes.prop_cycle'] = cycler(color=[cc[0], cc[0], cc[1], cc[1], cc[2], cc[2]],
                                         ls=['-','--','-','--','-','--'],
                                         marker=['o',]*6)
...
for mod_format in mod_formats:
    ...
    fig.suptitle(f"{mod_format} -- Rate", fontsize=16)
    for ax, l_link in zip(axs.flatten(), l_link_list):
        plot_data(ax,cnn_paths+tvrnn_paths, cnn_x+tvrnn_x, cnn_y+tvrnn_y, cnn_labels+tvrnn_labels, cnn_ss+tvrnn_ss, [[l_link]]*6)
        ax.grid()
        ax.set_title(f"link length {l_link} km")
        ax.set_xlim(-5,11)
    for i in range(3): axs[1][i].set_xlabel('SNR [dB]')
