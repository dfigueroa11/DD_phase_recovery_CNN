    ...
    for batch_size in batch_size_per_epoch:
        for i in range(batches_per_epoch):
            ...
            if (i+1)%(batches_per_epoch//checkpoint_per_epoch) == 0:
                SER = hlp.decode_and_ER(phase_diff.detach().cpu(),y_hat[:,:,1:].detach().cpu())
...
def eval_n_save_CNN():
    ...
    phase_diff = hlp.abs_phase_diff(x)
    SER = hlp.decode_and_ER(phase_diff,y_hat[:,:,1:])
    ...
    plt.hist(phase_diff.flatten(), 20, alpha=1, label='ideal DD')
