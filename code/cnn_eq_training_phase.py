    ...
    for batch_size in batch_size_per_epoch:
        for i in range(batches_per_epoch):
            ...
            if (i+1)%(batches_per_epoch//checkpoint_per_epoch) == 0:
                _, SER = hlp.decode_and_ER(phase.detach().cpu(),y_hat.detach().cpu())
...
def eval_n_save_CNN():
    ...
    if SNR_dB in SNR_save_fig and lr in lr_save_fig and L_link in L_link_save_fig and alpha in alpha_save_fig:
        ...
        _, _, hist1 = plt.hist(y[:,:,0::2].flatten(), 200, alpha=0.5)
        _, _, hist2 = plt.hist(y_hat.flatten(), 200, alpha=0.5)
        plt.ylim(0,2e4)
        plt.legend([line, hist1[0], hist2[0]],['ideal phase', 'DD out', 'CNN out'], loc='upper right')
